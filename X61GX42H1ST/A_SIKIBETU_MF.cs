...
{
    public partial class A_SIKIBETU_MF : Form
    {
        // プライベート変数
        private static A_SIKIBETU_MF m_Dlg = null;

        /// <summary>
        /// クラスのコンストラクタ
        /// </summary>
        public A_SIKIBETU_MF()
        {
            InitializeComponent();
        }
        ...
        /// フォームの初期化
        /// </summary>
        private void A_SIKIBETU_MF_Load(object sender, EventArgs e)
        {
            int i;   // カウンター変数
            ...
            c1FlexGrid1.Rows.Count              = 1001;                 // 251 行の総数
                    ...
                    using (OleDbCommand cmd = new OleDbCommand(SQl, DB))    // 検索条件をセット
                    {
                        using (OleDbDataReader RS = cmd.ExecuteReader())
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // 更新
                                    SQl  = "UPDATE T_識別コード SET ";
                                    SQl += "識別コード = '" + " " + c1FlexGrid1.GetData(i, 1) + "' ";
                        ...
                        using (OleDbDataReader RS = cmd.ExecuteReader())
                        {
                            ...
                            while (RS.Read()) {
                                if(RS["識別コード"].ToString().Length > 1)
                                {
                                    ...
                                    c1FlexGrid1.SetData(row, 1, Strings.Right(RS["識別コード"].ToString(), Strings.Len(RS["識別コード"]) - 1));
                                }
                            }
                        }
                    }
