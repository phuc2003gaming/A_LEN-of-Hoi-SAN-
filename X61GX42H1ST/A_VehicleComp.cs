...
namespace X61GX42H1ST
{
    public partial class A_VehicleComp : Form
    {
        ...
        // 照合結果変数
        private A_MAIN.VEHICLE_COMP  m_VehicleComp;
        private int                  m_Index;
        ...
        /// クラスのコンストラクタ
        /// </summary>
        public A_VehicleComp(A_PlcComp parent, A_MAIN.VEHICLE_COMP vehiclecomp, int idx)
        {
            InitializeComponent();
            ...
            m_Active = null;
            // 照合結果コピー
            m_VehicleComp = new A_MAIN.VEHICLE_COMP(vehiclecomp);
            m_Index       = idx;
        ...
        private void CompDisp()
        {
            ...
            if(ret == A_MAIN.COMP_ID.ONLYSIDE) {
            ...
            // ＬＨ側
            {
                ...
                if(srchas == true)
                {
                    ...
                    m_SrcLhdat[ 7].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Armrest      , m_VehicleComp.SrcDat[m_Index].LhDat.Armrest     );
                    m_SrcLhdat[ 8].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tumble       , m_VehicleComp.SrcDat[m_Index].LhDat.Tumble      );
                    m_SrcLhdat[ 9].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.IsoFix       , m_VehicleComp.SrcDat[m_Index].LhDat.IsoFix      );
                    m_SrcLhdat[10].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tether       , m_VehicleComp.SrcDat[m_Index].LhDat.Tether      );
                    m_SrcLhdat[11].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Ottoman      , m_VehicleComp.SrcDat[m_Index].LhDat.Ottoman     );
                    m_SrcLhdat[12].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.ConvHook     , m_VehicleComp.SrcDat[m_Index].LhDat.ConvHook    );
                    m_SrcLhdat[13].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.SideTable    , m_VehicleComp.SrcDat[m_Index].LhDat.SideTable   );
                    m_SrcLhdat[14].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Robot        , m_VehicleComp.SrcDat[m_Index].LhDat.Robot       );
                    // レシピ部
                    cnt = 0;
                ...
                if (dsthas == true)
                {
                    ...
                    m_DstLhdat[ 7].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Armrest      , m_VehicleComp.DstDat[m_Index].LhDat.Armrest     );
                    m_DstLhdat[ 8].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tumble       , m_VehicleComp.DstDat[m_Index].LhDat.Tumble      );
                    m_DstLhdat[ 9].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.IsoFix       , m_VehicleComp.DstDat[m_Index].LhDat.IsoFix      );
                    m_DstLhdat[10].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tether       , m_VehicleComp.DstDat[m_Index].LhDat.Tether      );
                    m_DstLhdat[11].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Ottoman      , m_VehicleComp.DstDat[m_Index].LhDat.Ottoman     );
                    m_DstLhdat[12].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.ConvHook     , m_VehicleComp.DstDat[m_Index].LhDat.ConvHook    );
                    m_DstLhdat[13].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.SideTable    , m_VehicleComp.DstDat[m_Index].LhDat.SideTable   );
                    m_DstLhdat[14].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Robot        , m_VehicleComp.DstDat[m_Index].LhDat.Robot       );
                    // レシピ部
                    cnt = 0;
                    for (int stno = 1; stno <= 16; stno++) {
                        for (int kk = 0; kk < 25; kk++) {
                ...
                {
                    // 照合結果
                    A_MAIN.COMP_ID comp = A_MAIN.COMP_ID.NONE;
                    if     ((srcrp[ii].Recipe == 0) != (dstrp[ii].Recipe == 0)) { comp = A_MAIN.COMP_ID.ONLYSIDE; }
                        ...
                        for(int stno = 1; stno <= 16; stno++)
                        {
                               ...
                               (dstrp[ii].St[stno - 1] > 0))
                            {
                                ...
                                {
                                    comp = A_MAIN.COMP_ID.MISMATCH;
                                }
                                else
                                {
                                    comp = A_MAIN.COMP_ID.MATCH;
                                }
                            }
                        }
                    }
            ...
            // ＲＨ側
            {
                ...
                if(srchas == true)
                {
                    ...
                    m_SrcRhdat[ 7].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Armrest      , m_VehicleComp.SrcDat[m_Index].RhDat.Armrest     );
                    m_SrcRhdat[ 8].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tumble       , m_VehicleComp.SrcDat[m_Index].RhDat.Tumble      );
                    m_SrcRhdat[ 9].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.IsoFix       , m_VehicleComp.SrcDat[m_Index].RhDat.IsoFix      );
                    m_SrcRhdat[10].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tether       , m_VehicleComp.SrcDat[m_Index].RhDat.Tether      );
                    m_SrcRhdat[11].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Ottoman      , m_VehicleComp.SrcDat[m_Index].RhDat.Ottoman     );
                    m_SrcRhdat[12].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.ConvHook     , m_VehicleComp.SrcDat[m_Index].RhDat.ConvHook    );
                    m_SrcRhdat[13].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.SideTable    , m_VehicleComp.SrcDat[m_Index].RhDat.SideTable   );
                    m_SrcRhdat[14].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Robot        , m_VehicleComp.SrcDat[m_Index].RhDat.Robot       );
                    // レシピ部
                    cnt = 0;
                    for (int stno = 1; stno <= 16; stno++) {
                        for (int kk = 0; kk < 25; kk++) {
                ...
                if (dsthas == true)
                {
                    ...
                    m_DstRhdat[ 7].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Armrest      , m_VehicleComp.DstDat[m_Index].RhDat.Armrest     );
                    m_DstRhdat[ 8].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tumble       , m_VehicleComp.DstDat[m_Index].RhDat.Tumble      );
                    m_DstRhdat[ 9].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.IsoFix       , m_VehicleComp.DstDat[m_Index].RhDat.IsoFix      );
                    m_DstRhdat[10].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Tether       , m_VehicleComp.DstDat[m_Index].RhDat.Tether      );
                    m_DstRhdat[11].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Ottoman      , m_VehicleComp.DstDat[m_Index].RhDat.Ottoman     );
                    m_DstRhdat[12].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.ConvHook     , m_VehicleComp.DstDat[m_Index].RhDat.ConvHook    );
                    m_DstRhdat[13].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.SideTable    , m_VehicleComp.DstDat[m_Index].RhDat.SideTable   );
                    m_DstRhdat[14].Text = Danpre_Xls_sub.GetMasterStr(Danpre_Xls_sub.M_ID.Robot        , m_VehicleComp.DstDat[m_Index].RhDat.Robot       );
                    // レシピ部
                    cnt = 0;
                    for (int stno = 1; stno <= 16; stno++) {
                        for (int kk = 0; kk < 25; kk++) {
        ...
        ***********************************************************************/
        public struct SORT_RECIPE
        {
            ...
            public A_MAIN.COMP_ID Comp;          // 照合結果
