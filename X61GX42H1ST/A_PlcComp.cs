...
namespace X61GX42H1ST
{
    public partial class A_PlcComp : Form
    {
        ...
        // 照合結果変数
        private A_MAIN.TAG_COMP      m_TagComp;
        ...
        /// クラスのコンストラクタ
        /// </summary>
        public A_PlcComp(A_MAIN parent, A_MAIN.TAG_COMP tagcomp, A_MAIN.VEHICLE_COMP vehiclecomp)
        {
            InitializeComponent();
            ...
            m_Active = null;
            // 照合結果コピー
            m_TagComp     = new A_MAIN.TAG_COMP(tagcomp);
        ...
        private void VehicleCompDisp()
        {
            ...
            for (int ii = 0; ii < m_VehicleComp.SrcDat.Length; ii++)
            {
                ...
                // 色表示
                A_MAIN.COMP_ID ret = A_MAIN.COMP_ID.NONE;
                do
                {
                    // ヘッダ部
                    {
                        for (int kk = 0; kk < 6; kk++)
                        {
                            ...
                            if ((ret == A_MAIN.COMP_ID.ONLYSIDE) && (kk == 0)) break;
                            else if (ret > A_MAIN.COMP_ID.MATCH) { ret = A_MAIN.COMP_ID.MISMATCH; break; }
                        }
                        // 処理中断
                        if (ret > A_MAIN.COMP_ID.MATCH) break;
                    }
                    ...
                    // ＬＨ側詳細部
                    {
                        for (int kk = 0; kk < 14; kk++)
                        {
                            ...
                            if (ret > A_MAIN.COMP_ID.MATCH) { ret = A_MAIN.COMP_ID.MISMATCH; break; }
                        }
                        // 処理中断
                        if (ret > A_MAIN.COMP_ID.MATCH) break;
                    ...
                    // ＲＨ側詳細部
                    {
                        ...
                        for (int stno = 1; stno <= 16; stno++)    // st1-> st16
                        {
                            for(int hh = 0; hh < 25; hh++)
                            {
                                ...
                                if (ret > A_MAIN.COMP_ID.MATCH) { ret = A_MAIN.COMP_ID.MISMATCH; break; }
                                else if(ret == A_MAIN.COMP_ID.NONE) break;
                            }
                            // 処理中断
                            if (ret > A_MAIN.COMP_ID.MATCH) break;
                        }
        ...
        private void DispModeChange()
        {
            ...
            for (int row = TAG_c1FlexGrid.Rows.Fixed; row < TAG_c1FlexGrid.Rows.Count; row++)
            {
                ...
                if (cell != null)
                {
                    if ((cell.StyleNew.BackColor != GetColor(A_MAIN.COMP_ID.MISMATCH)) &&
                        (cell.StyleNew.BackColor != GetColor(A_MAIN.COMP_ID.ONLYSIDE)))
                    {
        ...
        // 照合結果色
        public static Color GetColor(A_MAIN.COMP_ID flg)
        {
            switch (flg)
            {
                case A_MAIN.COMP_ID.MATCH:    return Color.White;
                case A_MAIN.COMP_ID.MISMATCH: return Color.Yellow;
                case A_MAIN.COMP_ID.ONLYSIDE: return Color.Yellow;
