...
//---------------------------------------------------------------------------------------
        ...
        ***********************************************************************/
        ...
        public struct DATA
        {
            ...
            public UInt16    Armrest;                   // アームレスト
            public UInt16    Tumble;                    // タンブル
            public UInt16    IsoFix;                    // ISO-FIX
            public UInt16    Tether;                    // テザー
            public UInt16    Ottoman;                   // オットマン
            public UInt16    ConvHook;                  // コンビニフック
            public UInt16    SideTable;                 // サイドテーブル
            public UInt16    Robot;                     // ロボット№
            ...
            public DATA(Nullable<DATA> buf)
            {
                ...
                if (buf.HasValue == true) {
                    ...
                    Tumble       = buf.Value.Tumble      ;
                    IsoFix       = buf.Value.IsoFix      ;
                    Tether       = buf.Value.Tether      ;
                    Ottoman      = buf.Value.Ottoman     ;
                    ConvHook     = buf.Value.ConvHook    ;
                    SideTable    = buf.Value.SideTable   ;
